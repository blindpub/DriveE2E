...
class B2D_VAD_Dataset(Custom3DDataset):
    ...
    def __init__(self, queue_length=4, bev_size=(200, 200),overlap_test=False,with_velocity=True,sample_interval=5,name_mapping= None,eval_cfg = None, map_root =None,map_file=None,past_frames=2, future_frames=6,point_cloud_range = [-51.2, -51.2, -5.0, 51.2, 51.2, 3.0] ,polyline_points_num=20,*args, **kwargs):
        ...
        ## DriveE2E: Find scene with empty lane
        # from tqdm import tqdm
        # empty_map_scene_list = []
        # for ii in tqdm(range(len(self.data_infos))):
        #     gt_labels , gt_bboxes = self.get_map_info(ii)
        #     if len(gt_bboxes.instance_list) == 0:
        #         if self.data_infos[ii]['folder'] not in empty_map_scene_list:
        #             empty_map_scene_list.append(self.data_infos[ii]['folder'])
        # print("Empty Scenes: ", empty_map_scene_list)
