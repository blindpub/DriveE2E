...
class PansegformerHead(SegDETRHead):
    ...
    def forward_test(self,
        ...
        with torch.no_grad():
            ...
            # ## DriveE2E Visualization
            # import cv2
            # import os
            # save_root = "./work_space/20240926/drivee2e_new_map_opt"
            # route_name = img_metas[0]['filename'][0].split('/')[4]

            # save_path = os.path.join(save_root, 'drivable_gt_vis_' + route_name + '_' + str(img_metas[0]['frame_idx'])+'.png')
            # gt_segmentation_vis = drivable_gt
            # gt_segmentation_vis = gt_segmentation_vis / gt_segmentation_vis.max() * 255
            # cv2.imwrite(save_path, gt_segmentation_vis.cpu().numpy())

            # save_path = os.path.join(save_root, 'drivable_pred_vis_' + route_name + '_' + str(img_metas[0]['frame_idx'])+'.png')
            # gt_segmentation_vis = drivable_pred
            # gt_segmentation_vis = gt_segmentation_vis / gt_segmentation_vis.max() * 255
            # cv2.imwrite(save_path, gt_segmentation_vis.cpu().numpy())

            # save_path = os.path.join(save_root, 'lanes_gt_vis_' + route_name + '_' + str(img_metas[0]['frame_idx'])+'.png')
            # gt_segmentation_vis = lanes_gt
            # gt_segmentation_vis = gt_segmentation_vis / gt_segmentation_vis.max() * 255
            # cv2.imwrite(save_path, gt_segmentation_vis.cpu().numpy())

            # save_path = os.path.join(save_root, 'lanes_pred_vis_' + route_name + '_' + str(img_metas[0]['frame_idx'])+'.png')
            # gt_segmentation_vis = lanes_pred
            # gt_segmentation_vis = gt_segmentation_vis / gt_segmentation_vis.max() * 255
            # cv2.imwrite(save_path, gt_segmentation_vis.cpu().numpy())
