...
class UniAD(UniADTrack):
    ...
    def forward_test(self,
        ...
        if self.with_occ_head:
            ...
            # import cv2
            # save_root = "./work_space/20240916/nuscenes"
            # route_name = img_metas[0]['filename'][0].split('/')[3]
            # save_path = os.path.join(save_root, 'occ_gt_vis_' + route_name + '_' + str(img_metas[0]['frame_idx'])+'.png')
            # seg_gt = outs_occ['seg_gt'].squeeze(2)
            # gt_segmentation_vis = seg_gt.sum(dim=1)
            # gt_segmentation_vis = gt_segmentation_vis / gt_segmentation_vis.max() * 255
            # gt_segmentation_vis = gt_segmentation_vis.reshape(200, 200)
            # cv2.imwrite(save_path, gt_segmentation_vis.cpu().numpy())

            # save_path = os.path.join(save_root, 'occ_pred_vis_' + route_name + '_' + str(img_metas[0]['frame_idx'])+'.png')
            # seg_gt = outs_occ['seg_out'].squeeze(2)
            # gt_segmentation_vis = seg_gt.sum(dim=1)
            # gt_segmentation_vis = gt_segmentation_vis / gt_segmentation_vis.max() * 255
            # gt_segmentation_vis = gt_segmentation_vis.reshape(200, 200)
            # cv2.imwrite(save_path, gt_segmentation_vis.cpu().numpy())
