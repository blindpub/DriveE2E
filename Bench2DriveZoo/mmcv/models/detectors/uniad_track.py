...
class UniADTrack(MVXTwoStageDetector):
    ...
    def _forward_single_frame_inference(
        ...
        # save_path = os.path.join(save_root, 'bev_embed_vis_' + route_name + '_' + str(img_metas[0]['frame_idx'])+'.png')
        # bev_embed_vis = bev_embed.reshape(200, 200, 1, 256).squeeze(2)
        # bev_embed_vis = bev_embed_vis.abs().sum(dim=2)
        # bev_embed_vis = bev_embed_vis / bev_embed_vis.max() * 255
        # cv2.imwrite(save_path, bev_embed_vis.cpu().numpy())
