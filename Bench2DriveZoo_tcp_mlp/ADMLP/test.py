...
config = GlobalConfig()
net = ADMLP(config)
ckpt = torch.load('log/drivee2e_final_gpu1/last-v3.ckpt', map_location="cuda")
...
config.val_data = 'admlp_drivee2e_final-val.npy'
...
with torch.no_grad():
	for index, batch in enumerate(tqdm(val_loader)):
		...
		l2_25.extend(np.linalg.norm(predict[:, 4, :2].detach().cpu().numpy() - waypoints[:, 4, :2].numpy(), axis=1).tolist())
		l2_3.extend(np.linalg.norm(predict[:, 5, :2].detach().cpu().numpy() - waypoints[:, 5, :2].numpy(), axis=1).tolist())
		# print(waypoints.shape)
print(sum(l2_05)/length)
print(sum(l2_1)/length)
print(sum(l2_15)/length)
print(sum(l2_2)/length)
print(sum(l2_25)/length)
print(sum(l2_3)/length)
print((sum(l2_05)/length + sum(l2_1)/length + sum(l2_15)/length + sum(l2_2)/length)/4)
print((sum(l2_05)/length + sum(l2_1)/length + sum(l2_15)/length + sum(l2_2)/length + sum(l2_25)/length + sum(l2_3)/length)/6)
