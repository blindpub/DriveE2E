...
from TCP.data import CARLA_Data, Drivee2e_Data
from TCP.config import GlobalConfig, GlobalConfig_Drivee2e
...
class TCP_planner(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # B = batch['target_command'].shape[0]
        # batch_steer_l1 = 0 
        # batch_brake_l1 = 0
        # batch_throttle_l1 = 0
        # for i in range(B):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--id', type=str, default='drivee2e', help='Unique experiment identifier.')
    ...
    # Config
    # config = GlobalConfig()
    config = GlobalConfig_Drivee2e()

    # Data
    train_set = Drivee2e_Data(root=config.root_dir_all, data_path=config.train_data, img_aug = config.img_aug)
    print(len(train_set))
    val_set = Drivee2e_Data(root=config.root_dir_all, data_path=config.val_data,)
    print(len(val_set))
