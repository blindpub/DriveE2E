...
class ADMLPAgent(autonomous_agent.AutonomousAgent):
	def setup(self, path_to_conf_file,routes_path):
		self.track = autonomous_agent.Track.SENSORS
		self.data_queue = deque()
		# self.data_queue_len = 21 ### 20 Hz!!!
		self.data_queue_len = 11 ### 10 Hz!!!
		...
		if SAVE_PATH is not None and SAVE_PATH != 'None':
			...
			string = pathlib.Path(routes_path).stem + '_'
	...
	def run_step(self, input_data, timestamp):
		...
		if self.step < self.data_queue_len:
			...
			if SAVE_PATH is not None and SAVE_PATH != 'None':
				self.save_only_img(tick_data)
			return control
		
		# if (self.step -  self.data_queue_len) % 10 != 0:
		# 	return self.last_control
		if (self.step -  self.data_queue_len) % 5 != 0:
			return self.last_control
		...
		for index in range(len(self.data_queue)-1, len(self.data_queue)-1-10-1, -2):
			# print(index)
			R = np.array([
		...
		# if SAVE_PATH is not None and SAVE_PATH != 'None' and (self.step -  self.data_queue_len) % 1 == 0:
		if SAVE_PATH is not None and SAVE_PATH != 'None' and (self.step -  self.data_queue_len) % 1 == 0:
			self.save(tick_data)
		self.last_control = control
	...
	def save_only_img(self, tick_data):
		frame = self.step
