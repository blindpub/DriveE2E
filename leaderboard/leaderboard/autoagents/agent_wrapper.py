...
MAX_ALLOWED_RADIUS_SENSOR={'bev':100.0, 'others':3.0}
...
def validate_sensor_configuration(sensors, agent_track, selected_track):
    ...
    for sensor in sensors:
        ...
        if 'x' in sensor and 'y' in sensor and 'z' in sensor:
            max_allowed_radius = MAX_ALLOWED_RADIUS_SENSOR['others']
            if sensor['id'] == 'bev':
                max_allowed_radius = MAX_ALLOWED_RADIUS_SENSOR['bev']
                
            if math.sqrt(sensor['x']**2 + sensor['y']**2 + sensor['z']**2) > max_allowed_radius:
                raise SensorConfigurationInvalid(
                    "Illegal sensor extrinsics used for sensor '{}'. Max allowed radius is {}m".format(sensor['id'], max_allowed_radius))
...
class AgentWrapper(object):
    ...
    def _preprocess_sensor_spec_expert_data(self, sensor_spec):
        type_ = sensor_spec["type"]
        id_ = sensor_spec["id"]
        attributes = {}
    ...
    def setup_sensors(self, vehicle, use_predefined_route):
        """
        ...
        for sensor_spec in self._agent.sensors():
            if not use_predefined_route:
                type_, id_, sensor_transform, attributes = self._preprocess_sensor_spec(sensor_spec)
            else:
                type_, id_, sensor_transform, attributes = self._preprocess_sensor_spec_expert_data(sensor_spec)
