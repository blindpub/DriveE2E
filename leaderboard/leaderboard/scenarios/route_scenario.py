...
class RouteScenario(BasicScenario):
    ...
    def __init__(self, world, config, algo="expert",debug_mode=0, scenario_flag="DriveE2E", criteria_enable=True,
                 use_predefined_route=False, ego_predefined_route=None,ego_predefined_speed=None,
                 other_agent_setting=False, other_routes_dict=None):
        """
        ...
        ## DriveE2E: set ego route and other agent route
        self.scenario_flag = scenario_flag
        self.use_predefined_route = use_predefined_route
        self.ego_predefined_route = ego_predefined_route
        self.ego_predefined_speed = ego_predefined_speed
        self.other_agent_setting = other_agent_setting
        self.other_routes_dict = other_routes_dict
        ...
        ## DriveE2E: set ego route and other agent route
        if self.other_agent_setting:
            self.other_actors_dict = self._spawn_other_agents_tfd()
        else:
            self.other_actors_dict = None
    ...
    def _get_route(self, config):
        ...
        """
        if self.scenario_flag == "DriveE2E":
            ## DriveE2E: hardcode setting with original waypoints
            predefined_route_location = []
            total_waypoints_num = 0
            for ii in range(len(self.ego_predefined_route)):
                if self.ego_predefined_route[ii] is not None:
                    # predefined_route_location.append(self.ego_predefined_route[ii].location)
                    predefined_route_location.append(self.ego_predefined_route[ii])
                    total_waypoints_num += 1

            self.gps_route, self.route = interpolate_trajectory(predefined_route_location, True)

            if total_waypoints_num != len(self.route):
                print("There is a bug! total_waypoints_num: %s, len(self.route): %s"%(total_waypoints_num,len(self.route)))
        else:
    ...
    def _set_init_speed(self, actor, target_speed):
        transform = actor.get_transform()
        forward_vector = transform.get_forward_vector()

        velocity = carla.Vector3D(
            forward_vector.x * target_speed,
    ...
    ## DriveE2E: spawn other agents from TFD
    ## Need To Do: add more classes
    def _spawn_other_agents_tfd(self):
        """Spawn the other agents at the first waypoint of the route from TFD dataset"""
        if self.other_routes_dict is None:
            return None
        ...
        other_actors_dict = {}
        pid_actors_dict = {}
        for actor_id in list(self.other_routes_dict.keys()):
            if self.other_routes_dict[actor_id]['spawn_time'] == 0:
                spawn_flag = False
                if self.other_routes_dict[actor_id]['type'] == 'VEHICLE':
                    # vehicle_bp = blueprint_library.filter('vehicle')[0]
                    # print(vehicle_bp)

                    # # if actor_id in [200004, 200012, 200005, 200010, 200007, 200006]:
                    # if actor_id in [200004, 200012, 200005, 200010, 200007, 200006]:
                    #     vehicle_bp = blueprint_library.filter('vehicle')[0]
                    #     print('audi!')
                    # else:
                    #     vehicle_bp = blueprint_library.find(self.other_routes_dict[actor_id]['blueprint'])
                    #     print(f'{actor_id}: {vehicle_bp}')
                    vehicle_bp = blueprint_library.find(self.other_routes_dict[actor_id]['blueprint'])
                    
                    spawn_point = self.other_routes_dict[actor_id]['routes'][0]
                    try:
                        vehicle = self.world.spawn_actor(vehicle_bp, spawn_point)
                        
                        if vehicle_bp.id.startswith('vehicle') and self.other_routes_dict[actor_id]['real_time'] > 18.5:
                            light_state = carla.VehicleLightState(carla.VehicleLightState.HighBeam |
                                carla.VehicleLightState.LowBeam |
                                carla.VehicleLightState.Position)
                            vehicle.set_light_state(light_state)
                            
                        other_actors_dict[actor_id] = vehicle
                        spawn_flag = True
                    except Exception as e:
                        print(f"Failed to spawn actor {actor_id}: {e}")
                        del self.other_routes_dict[actor_id]
                        print(f"Removed actor_id {actor_id} from routes_dict")
                        spawn_flag = False

                elif self.other_routes_dict[actor_id]['type'] == 'BICYCLE':
                    bicycle_bp = blueprint_library.find(self.other_routes_dict[actor_id]['blueprint'])
                    
                    spawn_point = self.other_routes_dict[actor_id]['routes'][0]
                    try:
                        bicycle = self.world.spawn_actor(bicycle_bp, spawn_point)
                        other_actors_dict[actor_id] = bicycle
                        spawn_flag = True
                    except Exception as e:
                        print(f"Failed to spawn actor {actor_id}: {e}")
                        del self.other_routes_dict[actor_id]
                ...
                elif self.other_routes_dict[actor_id]['type'] == 'PEDESTRIAN':
                    ped_bp = blueprint_library.find(self.other_routes_dict[actor_id]['blueprint'])
                    
                    spawn_point = self.other_routes_dict[actor_id]['routes'][0]
                    try:
                        pedstrain = self.world.spawn_actor(ped_bp, spawn_point)
                        other_actors_dict[actor_id] = pedstrain
                        spawn_flag = True
                    except Exception as e:
                        print(f"Failed to spawn actor {actor_id}: {e}")
                        del self.other_routes_dict[actor_id]
                ...
                if spawn_flag:
                    target_speed = float(self.other_routes_dict[actor_id]['target_speed'][0])
                    
                    if self.algo != "expert":
                        other_actors_dict[actor_id] = self._set_init_speed(other_actors_dict[actor_id],target_speed)
                        # print(f"actor_id: {actor_id} target_speed: {target_speed}")
                                        
        return other_actors_dict

    def _spawn_ego_vehicle(self):
        """Spawn the ego vehicle at the first waypoint of the route"""
        elevate_transform = self.route[0][0]
        if self.scenario_flag != "DriveE2E":
            elevate_transform.location.z += 0 # origin value: 0.5
        ...
        ## DriveE2E: need to check why we need + carla.Location(z=50), and carla.Rotation(pitch=-90)
        spectator.set_transform(carla.Transform(elevate_transform.location + carla.Location(z=50),
                                                    carla.Rotation(pitch=-90)))
                        

        #set init speed
        target_speed = float(self.ego_predefined_speed[0])
        
        if self.algo != "expert":
            ego_vehicle = self._set_init_speed(ego_vehicle,target_speed)
        # print(f"ego_vehicle target_speed: {target_speed}")
    ...
    def _get_parking_slots(self, max_distance=100, route_step=10):
        ...
        for scenario in self.list_scenarios:
            self.occupied_parking_locations.extend(scenario.get_parking_slots())
    ...
    def _create_behavior(self):
        ...
        ## DriveE2E: hardcode to remove the backgrounnd activity
    ...
    def _create_test_criteria(self):
        ...
        # End condition
        criteria.add_child(RouteCompletionTest(self.ego_vehicles[0], route=self.route, use_predefined_route=self.use_predefined_route))
