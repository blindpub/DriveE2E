...
def interpolate_trajectory(waypoints_trajectory, drivee2e_scenario_setting = False,hop_resolution=1.0):
    """
    ...
    route = []
    gps_route = []
    if drivee2e_scenario_setting:
        ### DriveE2E  
        for i in range(len(waypoints_trajectory)):
            cur_trans = waypoints_trajectory[i]
            if cur_trans is not None:                    
                connection = RoadOption.LANEFOLLOW
                route.append((cur_trans,connection))
                
                gps_coord = _location_to_gps(lat_ref, lon_ref, cur_trans.location)
