...
class RouteParser(object):
    ...
    @staticmethod
    def parse_routes_file(scenario_flag,route_filename, routes_subset='', route2weather_file=''):
        """
        ...
        with open(route2weather_file,'r') as f:
            route2weather = json.load(f)
        ...
        for route in tree.iter("route"):
            ...
            route_config.weather_id = route2weather[route_id]

            ## DriveE2E: add new attribute
            if scenario_flag == "DriveE2E":
                try:
                    route_config.scenario_csv_id = route.attrib['scenario_csv_id']
                    route_config.ego_id = route.attrib['ego_id']

                    tmp_name = route.attrib['scenario_csv_id'].split('_')[0]
                    tmp_routename = "Route{}".format(route_id)
                    tmp_weathername = "Weather{}".format(route2weather[route_id])
                    
                    route_config.collect_data_save_name = tmp_name+"_"+route.attrib['town']+"_"+tmp_routename+"_"+tmp_weathername
                except:
                    print("This is not DriveE2E!")
