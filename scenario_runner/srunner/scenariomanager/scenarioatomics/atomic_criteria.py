        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            ...
            if self._current_index != closest_index:
                ...
                if not self._in_safe_route:
                    ...
                    if self._accum_meters[-1] == 0:
                        out_route_percentage = 100
                    else:
...
class RouteCompletionTest(Criterion):
    ...
    PERCENTAGE_THRESHOLD = 97 #99  # %   route_len: 38-100

    def __init__(self, actor, route, name="RouteCompletionTest", use_predefined_route=False, terminate_on_failure=False):
        """
        """
    ...
    def _get_acummulated_percentages(self):
        ...
        if max_dist == 0:
            return [0 for _ in accum_meters]
        else:
    ...
    def update(self):
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            ...
            if self.use_predefined_route:
                ...
                self._count += 1
                if self._count == self._route_length:
                    comp_flag = True
            else:
