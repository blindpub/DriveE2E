...
import h5py
import laspy
from tools.utils import build_projection_matrix, convert_depth, get_relative_transform, normalize_angle, build_skeleton,  get_matrix, calculate_cube_vertices, compute_2d_distance
from tools.utils import DIS_CAR_SAVE, DIS_WALKER_SAVE, DIS_SIGN_SAVE, DIS_LIGHT_SAVE
...
class Env_Manager(object):
    def __init__(self,town,data_save_name):
        self.town = town
        
        if SAVE_PATH is not None and SAVE_PATH != 'None':
            self.save_path = pathlib.Path(os.path.join(os.environ['SAVE_PATH'],data_save_name))
    ...
    def create_save_path(self):
        (self.save_path / 'camera' / 'rgb_front').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'rgb_front_left').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'rgb_front_right').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'rgb_back').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'rgb_back_left').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'rgb_back_right').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'rgb_top_down').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'semantic_front').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'semantic_front_left').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'semantic_front_right').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'semantic_back').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'semantic_back_left').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'semantic_back_right').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'instance_front').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'instance_front_left').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'instance_front_right').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'instance_back').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'instance_back_left').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'instance_back_right').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'depth_front').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'depth_front_left').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'depth_front_right').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'depth_back').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'depth_back_left').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'camera' / 'depth_back_right').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'radar').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'lidar').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'anno').mkdir(parents=True, exist_ok=True)
        (self.save_path / 'expert_assessment').mkdir(parents=True, exist_ok=True)
    ...
    def tick(self, input_data):
        ...
        should_brake = True if control.brake > 0 else False
        # waypoint = self.map.get_waypoint(self.last_ego_transform.location)
    ...
    def get_bounding_boxes(self, lidar=None, radar=None):
        ...
        result = {
            ...
            'color': npc.attributes.get('color','default_color'),
            'speed': ego_speed,
            'brake': ego_brake,
            'road_id': road_id,
            'lane_id': lane_id,
    ...
    def update_command(self):
        delta = 6
        cur_trans = np.array([self._global_plan_world_coord_src[self.count][0].location.x,self._global_plan_world_coord_src[self.count][0].location.y])
        
        next_id = self.count + delta
        if next_id < len(self._global_plan_world_coord_src):
            next_trans = np.array([self._global_plan_world_coord_src[next_id][0].location.x,self._global_plan_world_coord_src[next_id][0].location.y])
        else:
            next_trans = None
            
        last_id = self.count - delta
        if last_id >= 0:
            last_trans = np.array([self._global_plan_world_coord_src[last_id][0].location.x,self._global_plan_world_coord_src[last_id][0].location.y])
        else:
            last_trans = None 
        
        if last_trans is  None or next_trans is None:
            #Lanefollow
            command = 4
        else:
            def determine_turn(point1, point2, point3):
                # Extract coordinates
                x1, y1 = point1
                x2, y2 = point2
                x3, y3 = point3
                ...
                angle = math.degrees(math.acos(max(min(cos_angle, 1), -1)))  # 夹角（度数）

                # Determine direction based on the sign of the cross product
                if z_cross < 0:
                    angle = -angle  # 如果是右转，角度取负值
            ...
            command = determine_turn(last_trans, cur_trans, next_trans)
    ...
    def update_speed(self):
        #
        # cur_trans = np.array(self._global_plan_world_coord_src[self.count][0].location.x,self._global_plan_world_coord_src[self.count][0].location.y,self._global_plan_world_coord_src[self.count][0].location.z)
        cur_trans = np.array([self._global_plan_world_coord_src[self.count][0].location.x,self._global_plan_world_coord_src[self.count][0].location.y])
        # cur_trans_gps = np.array(self._global_plan_gps_src[self.count][0]['lat'],self._global_plan_gps_src[self.count][0]['lon'],self._global_plan_gps_src[self.count][0]['z'])
        cur_trans_gps = np.array([self._global_plan_gps_src[self.count][0]['lat'],self._global_plan_gps_src[self.count][0]['lon']])
        cur_pos = self.gps_to_location(cur_trans_gps)
        
        next_id = self.count+1        
        if next_id < len(self._global_plan_world_coord_src):
            # next_trans = np.array(self._global_plan_world_coord_src[next_id][0].location.x,self._global_plan_world_coord_src[next_id][0].location.y,self._global_plan_world_coord_src[next_id][0].location.z)
            next_trans = np.array([self._global_plan_world_coord_src[next_id][0].location.x,self._global_plan_world_coord_src[next_id][0].location.y])
            # next_trans_gps = np.array(self._global_plan_gps_src[next_id][0]['lat'],self._global_plan_gps_src[next_id][0]['lon'],self._global_plan_gps_src[next_id][0]['z'])   
            next_trans_gps = np.array([self._global_plan_gps_src[next_id][0]['lat'],self._global_plan_gps_src[next_id][0]['lon']])          
            next_pos = self.gps_to_location(next_trans_gps)
            
            #10HZ    
            ego_velocity = (next_trans - cur_trans) / 0.1
            ego_velocity_gps = (next_pos - cur_pos) / 0.1
            
            #
            speed = np.linalg.norm(ego_velocity[0:2])
        else:
            speed = 0
    ...
    def run_step(self, input_data, timestamp):
        ...
        command_near_xy = np.array([tick_data['command_near_xy'][0]-tick_data['pos'][0],-tick_data['command_near_xy'][1]+tick_data['pos'][1]]) 
        ...
        tick_data['command_near_xy'] = np.array([local_command_xy[1],local_command_xy[0]])
        
        command = tick_data['command_near'].value
        command = self.update_command()

        tick_data['command_near'] = command
                 
        far_node = tick_data['command_near_xy']
        far_command = tick_data['command_near']
        tick_data['next_command'] = far_command
        
        tick_data['x_target'] = far_node[0]
        tick_data['y_target'] = far_node[1]
        
        #DriveE2E: update speed by self._global_plan_world_coord_src
        tick_data['speed'] = self.update_speed()
        
        npc = self.manager.ego_vehicles[0]
        location = npc.get_transform().location   
        tick_data['x'] = location.x
        tick_data['y'] = location.y
                        
        if SAVE_PATH is not None and SAVE_PATH != 'None':
            self.save(tick_data['command_near_xy'], far_node, tick_data['command_near'], far_command, tick_data, tick_data['should_brake'])
    ...
    def set_global_plan(self, global_plan_gps, global_plan_world_coord):
        ...
        self._global_plan_world_coord_src = global_plan_world_coord
        self._global_plan_gps_src = global_plan_gps

    def _weather_to_dict(self, weather_data):
        weather = {}           
        ...
        weather["fog_falloff"] = weather_data.fog_falloff
